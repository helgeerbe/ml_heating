...
def find_best_outlet_temp(model: Any, features: List[float], target_temp: float) -> float:
    min_temp, max_temp, step = 30.0, 55.0, 0.5
    best_temp, min_diff = 40.0, float("inf")

    # --- Correctly define feature indices ---
    # [outdoor, pv, tv]
    outlet_hist_start = 3
    outlet_hist_end = outlet_hist_start + HISTORY_STEPS
    
    indoor_hist_start = outlet_hist_end
    indoor_hist_end = indoor_hist_start + HISTORY_STEPS
    
    delta_outlet_start = indoor_hist_end
    ...
    for temp_candidate in np.arange(min_temp, max_temp+step, step):
        X_candidate_list = base_features[:]

        # 1. Create a new outlet history with the candidate temperature
        # The value to change is the *last* element of the outlet history
        new_outlet_history = X_candidate_list[outlet_hist_start:outlet_hist_end]
        new_outlet_history[-1] = temp_candidate

        # 2. Recalculate the delta for the new outlet history
        new_delta_outlet = np.diff(new_outlet_history, prepend=new_outlet_history[0]).tolist()

        # 3. Update the feature list with both the new history and the new delta
        ...
        try:
            predicted = model.predict(X_candidate)[0]
            if DEBUG:
                print(f"  - Testing outlet {temp_candidate:.1f}°C -> Predicted indoor: {predicted:.2f}°C")
        except Exception:
            continue
        diff = abs(predicted - target_temp)
        if diff < min_diff:
...
def initial_train_model(model: Any, lookback_hours: int = 6) -> Any:
    """
    Train the model on last `lookback_hours` of data for realistic first prediction.
    """
    ...
    for idx in range(max(outlet_steps, indoor_steps), len(df)):
        row = df.iloc[idx]
        ...
        outlet_hist = df.iloc[idx-outlet_steps:idx][hp_outlet_temp_id].ffill().bfill().tolist()
        indoor_hist = df.iloc[idx-indoor_steps:idx][kuche_temperatur_id].ffill().bfill().tolist()
        ...
        label = row.get(kuche_temperatur_id, None)
        if label is None:
            continue
    ...
    while True:
        try:
            ...
            if heating_state == 'heat' and last_state and actual_indoor is not None:
                try:
                    X_train = last_state["features"]
                    y_train = np.array([actual_indoor]).ravel()
                    if DEBUG:
                        print("--- Online Training ---")
                        print(f"Training with label (actual indoor): {y_train[0]:.2f}°C")
                        print(f"Feature vector: {X_train.flatten().tolist()}")
            ...
            if not is_fitted or model.n_features_in_ != len(current_features_list):
                suggested_temp = 40.0
                predicted_indoor = float(actual_indoor or 21.0)
            else:
                X_val = np.array(current_features_list).reshape(1, -1)
                predicted_indoor = float(model.predict(X_val)[0])
                suggested_temp = find_best_outlet_temp(model, current_features_list, TARGET_INDOOR_TEMP)
            ...
            print(f"{datetime.now().isoformat()} - Target: {TARGET_INDOOR_TEMP}°C | Suggested: {suggested_temp:.1f}°C | Predicted: {predicted_indoor:.2f}°C | Actual: {actual_indoor or 'N/A'}")
