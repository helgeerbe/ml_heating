...
def reproduce_morning_drop_v3():
    print("=== Reproducing Morning Drop Scenario V3 (Post-Blocking) ===")
    
    # 1. Setup Scenario
    # Morning transition: Cold but rising outdoor temp, Sun coming up
    # System just came out of DHW blocking
    current_outdoor = 5.0
    ...
    target_temp = 21.0
    current_indoor = 20.96 # Slightly below target (0.04 deficit), needs heating
    ...
    elif outlet_temp < 30.0 and current_indoor < target_temp:
        print("⚠️  FAILURE REPRODUCED: Underheating detected!")
        print(f"Explanation: Outlet temp {outlet_temp:.2f}°C is too low for cold house ({current_indoor}°C < {target_temp}°C).")
    else:
        print("✅ Morning drop is contained (or not reproduced).")

if __name__ == "__main__":
    reproduce_morning_drop_v3()
