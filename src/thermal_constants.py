...
class ThermalUnits:
    ...
    RANGES = {
        'thermal_time_constant': (PhysicsConstants.MIN_TIME_CONSTANT, 
                                PhysicsConstants.MAX_TIME_CONSTANT),
        'heat_loss_coefficient': (PhysicsConstants.MIN_HEAT_LOSS_COEFF,
                                PhysicsConstants.MAX_HEAT_LOSS_COEFF),
        'outlet_effectiveness': (PhysicsConstants.MIN_EFFECTIVENESS,
                               PhysicsConstants.MAX_EFFECTIVENESS),
        'indoor_temperature': (PhysicsConstants.MIN_BUILDING_TEMP,
                             PhysicsConstants.MAX_BUILDING_TEMP),
        'outdoor_temperature': (PhysicsConstants.MIN_OUTDOOR_TEMP,
                              PhysicsConstants.MAX_OUTDOOR_TEMP),
        'outlet_temperature': (PhysicsConstants.MIN_OUTLET_TEMP,
    ...
    def validate_parameter(cls, parameter_name: str, value: float) -> bool:
        ...
        if parameter_name not in cls.RANGES:
            logging.warning(f"No validation range defined for {parameter_name}")
            return True
    ...
    @classmethod
    def validate_parameters(cls, parameters: Dict[str, Any]) -> Dict[str, bool]:
        """
...
class ThermalParameterValidator:
    ...
    def validate_heat_balance_parameters(self, heat_loss_coeff: float, 
                                       outlet_effectiveness: float,
                                       external_weights: Dict[str, float]) -> bool:
        """
        ...
        if not ThermalUnits.validate_parameter('heat_loss_coefficient', heat_loss_coeff):
            self.validation_errors.append("Heat loss coefficient out of range")
        
        if not ThermalUnits.validate_parameter('outlet_effectiveness', outlet_effectiveness):
        ...
        for source, weight in external_weights.items():
            ...
            if not ThermalUnits.validate_parameter(param_name, weight):
                self.validation_errors.append(f"{source} heat weight out of range")
        
        # Physics consistency checks
        if heat_loss_coeff <= 0:
            self.validation_errors.append("Heat loss coefficient must be positive")
        
        if outlet_effectiveness <= 0:
            self.validation_errors.append("Outlet effectiveness must be positive")
    ...
    def validate_temperature_inputs(self, indoor: float, outdoor: float, 
                                  outlet: float) -> bool:
        """
